...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            annotator = Annotator(im0, line_width=2,font = 'Arial.ttf', pil=not ascii)
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        inside = cv2.pointPolygonTest(np.array(roi), (x,h), False)
                        if inside>0:
                          regionid.add(id)
                        
                        c = int(cls)  # integer class
                        label = f' {names[c]} {conf:.2f}'
                        cv2.putText(im0, "count =" + str(len(regionid)), (20, 50), 0, 1, (100, 200, 0), 2)
